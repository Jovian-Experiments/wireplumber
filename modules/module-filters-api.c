    ...
    const gchar *direction, gint target_id)
{
  ...
  /* Find the first group matching target_id */
  while (filters) {
...
reevaluate_groups_target (WpFiltersApi *self)
{
  ...
  for (; wp_iterator_next (it, &item); g_value_unset (&item)) {
    ...
    /* Get current target */
    curr_target = g_hash_table_lookup (self->groups_target, key);
...
update_values_from_metadata (WpFiltersApi * self, Filter *f)
{
  static const gchar * metadata_keys[2] = {
    [WP_DIRECTION_INPUT] = "filters.configured.inputs",
    [WP_DIRECTION_OUTPUT] = "filters.configured.outputs",
  };
  g_autoptr (WpMetadata) m = NULL;
  ...
  for (; wp_iterator_next (it, &item); g_value_unset (&item)) {
    ...
    if (g_str_equal (f_stream_name, stream_name) &&
      ...
      if (wp_spa_json_object_get (j, "group", "s", &group, NULL)) {
        if (!f->group || !g_str_equal (f->group, group)) {
          g_clear_pointer (&f->group, g_free);
      ...
      /* Update use */
      if (wp_spa_json_object_get (j, "use", "s", &use, NULL)) {
        if (g_str_equal (use, "always")) {
          if (!f->enabled) {
            f->enabled = TRUE;
            changed = TRUE;
          }
        } else if (g_str_equal (use, "never")) {
          if (f->enabled) {
            f->enabled = FALSE;
            changed = TRUE;
          }
        } else if (g_str_equal (use, "playback-only")) {
          if (f->enabled != (self->n_playback_stream_nodes > 0)) {
            f->enabled = self->n_playback_stream_nodes > 0;
            changed = TRUE;
          }
        } else if (g_str_equal (use, "capture-only")) {
          ...
          wp_warning_object (self,
              "The '%s' value is not a valid for the 'use' filter filed", use);
        }
      }
    ...
    const gchar *key, const gchar *type, const gchar *value, gpointer d)
{
  ...
  /* Reevaluate everything */
  if (reevaluate_groups_target (self))
    changed = TRUE;
  for (guint i = 0; i < 2; i++)
    if (reevaluate_filters (self, i))
      changed = TRUE;
