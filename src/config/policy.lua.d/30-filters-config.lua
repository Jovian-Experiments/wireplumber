...
default_policy.filters_metadata = {
  ["filters"] = {
    -- Input filters (meant to be linked with Audio/Sink device nodes)
    {
      ...
      ["use"] = "always",  -- can be 'always', 'never', 'capture-only' or 'playback-only'
      ["priority"] = 30,
    },
    {
    ...
    },
    {
      ...
      ["direction"] = "input",  -- can only be 'input' or 'output'
      ["group"] = "speakers",  -- if nil, the default node will be used as target
      ["use"] = "capture-only",  -- can be 'always', 'never', 'playback-only' or 'capture-only'
      ["priority"] = 10,
    },
    ...
    -- Output filters (meant to be linked with Audio/Source device nodes)
    {
      ...
      ["group"] = nil,  -- if nil, the default node will be used as target
      ["use"] = "always",  -- can be 'always', 'never', 'playback-only' or 'capture-only'
      ["priority"] = 30,
    },
    {
  ...
  -- The target node properties of each filter group
  ["targets"] = {
    ["speakers"] = {
